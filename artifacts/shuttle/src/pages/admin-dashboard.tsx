...
import { Users, CalendarCheck, Clock, TrendingUp, CheckCircle2, AlertCircle, XCircle, ArrowRight, Loader2, BarChart3, Shield } from "lucide-react";
                  ...
                  {demand?.map((trip, idx) => {
                    ...
                    return (
                      <tr key={trip.tripId} className={`${idx !== demand.length - 1 ? "border-b border-white/[0.04]" : ""} hover:bg-white/[0.02] transition-colors`}>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-20 h-1.5 bg-white/10 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-gradient-to-r from-[#ff2e88] to-[#ff2e88]/60 rounded-full"
                                style={{ width: `${fillPct}%` }}
                              />
                            </div>
                        ...
                        <td className="px-5 py-4">
                          <span className="text-sm font-mono text-emerald-400">{trip.confirmedCount}</span>
                        </td>
                        <td className="px-5 py-4">
                          ...
                          {trip.status === "pending" && (
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className="text-xs px-3 py-1.5 rounded-lg bg-emerald-400/10 text-emerald-400 border border-emerald-400/20 hover:bg-emerald-400/20 transition-colors"
                              >
                                Confirm
                              </button>
                              <button
                  ...
                  <div key={trip.tripId} className="p-4 space-y-3">
                    ...
                    {trip.status === "pending" && (
                      <div className="flex gap-2 pt-1">
                        <button onClick={() => handleConfirm(trip.tripId)} className="flex-1 py-2 rounded-lg text-xs font-medium bg-emerald-400/10 text-emerald-400 border border-emerald-400/20">
                          Confirm Trip
                        </button>
                        <button onClick={() => handleCancel(trip.tripId)} className="flex-1 py-2 rounded-lg text-xs font-medium bg-red-400/10 text-red-400 border border-red-400/20">
                          Cancel Trip
                        </button>
                      </div>
                    )}
